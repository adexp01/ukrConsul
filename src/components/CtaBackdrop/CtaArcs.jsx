 ...
 * лінії можна було дотягнутись анімацією.
 *
 * `preserveAspectRatio` перемикається за пропорцією бокса, бо жоден режим
 * не годиться на всі: при типовому `meet` лінії на широкій картці
 * закінчувались посеред неї; `none` на широкому боксі ідеальний (усі вісім
 * дуг видно, неоднорідний масштаб на майже прямих кривих не читається), але
 * на вузькому мобільному він розтягує їх по вертикалі й прогин стає надто
 * крутим. Тому вузькі бокси отримують `slice` — пропорція збережена,
 * обрізається по горизонталі, а лінії однаково йдуть від краю до краю. Геометрія і градієнти скопійовані
...
/** Нижче цієї пропорції бокса дуги краще вписувати `slice`, а не тягнути */
...
export const CtaArcs = ({ className = "" }) => {
  ...
  /* Пропорція, з якої `none` перестає бути кращим за `slice` */
  useEffect(() => {
    ...
    const apply = () => {
      ...
      root.setAttribute(
        "preserveAspectRatio",
        width / height < WIDE_ASPECT ? "xMidYMid slice" : "none",
      );
    };
